...
export default function Cart({icon, goRightIcon, firstItem, title, subtitle, breakIt, duration, src, src2, children}){
    ...
    const rightIcon = 
    <div onClick={handleCardClick} className={`${darkMode ? 'bg-black' : 'bg-cyan-200'} shadow-xl w-8 h-8 rounded-full translate-y-3 z-50 ${firstItem ? 'translate-x-1 w-[44px]' : 'translate-x-1'}`}>
        <h1 className={`${darkMode ? 'text-cyan-200' : 'text-black'} font-semibold text-lg`}><FontAwesomeIcon icon={icon} /></h1>
    </div>
